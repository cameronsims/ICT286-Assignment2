...
// Change user data 
function user_change($user_id, $user_type, $user_fname, $user_address, $user_phone, $user_email) {
	// Get login in an array
	$sql_login = sql_login();
	$sql_host = $sql_login[0];
	$sql_user = $sql_login[1];
...
function user_exec() {
	...
	$user_login = json_decode($_COOKIE["cs_ulog"], true);
	...
	if (isset($_GET["getall"]) && $admin_access) {
		...
		$login_cookie = json_decode($_COOKIE["cs_ulog"], true);
		$user = user_get($login_cookie["id"]);
	...
	if ($action == "login" && $has_nm && $has_pw) {
		...
		if ($success) {
			...
			setcookie("cs_ulog", "{\"id\":$user_id,\"username\":\"$user_name\",\"password\":\"$user_pw\", \"admin\": $is_staff}", $exp, '/');
		} 
	}
	...
	else if ($action == "edit" && $change_valid) {
		// Update 
		user_change($user_id, $user_type, $user_fname, $user_address, $user_phone, $user_email);
		user_run($user_id);
	}
	// If we want to delete an existing account
	else if ($action == "delete" && $valid_login) {
		// Delete
		user_delete($user_id);
	}
}

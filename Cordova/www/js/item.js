...
function cart_place() {
	...
	for (let i = 0; i < items.length; i++) {
		...
		eImage.attr("src", "https://eris.ad.murdoch.edu.au/~34829454/assignment-2/img/item/" + ID + ".jpeg");
		...
		let eDeleteD = $( document.createElement("td") );
		let eDelete = $( document.createElement("button") );
		eDelete.click(function(){
			// Remove item from cookie
			let cart = cookie_get("cs_shoppingcart");
			let json = JSON.parse(cart);
			let newJson = [];
			
			// For each item in the cart, delete if it is
			...
			cookie_set( "cs_shoppingcart", newJsonStr );
			cart_place();
		});
		
		eDelete.text("Remove From Cart");
		eDeleteD.append( eDelete );
		eRow.append(eDeleteD);
...
function item_orderCart() {
	...
	// Cookie 
	let c = "cs_ulog=" + cookie_get("cs_ulog") + "&cs_shoppingcart=" + cookie_get("cs_shoppingcart");
	document.cookie = "cs_ulog=" + cookie_get("cs_ulog") + ";cs_shoppingcart" + cookie_get("cs_shoppingcart") + ";";

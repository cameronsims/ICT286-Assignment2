...
function user_attemptLogin(elem, info, onload) {
	...
	let url = "https://eris.ad.murdoch.edu.au/~34829454/assignment-2/user.php";
	let query = "?action=login";
	...
	xhr.onreadystatechange = function() {
		...
		if (xhr.readyState == 4 && xhr.status == 200) {
			// Parse the JSON
			console.log(url + query, reqHeader, "->", xhr.responseText);
			let json = JSON.parse(xhr.responseText);
			
			
			// Set cordova cookies
			const login_cookie = json["cs_ulog"];
			cookie_set("cs_ulog", login_cookie);
...
function user_createAccount(elem, info, onload) {
	...
	// This is the url we're going to
	let url = "https://eris.ad.murdoch.edu.au/~34829454/assignment-2/user.php" + "?action=create";
...
function user_change(values) {
	...
	// This is the url we're going to
	let url = "https://eris.ad.murdoch.edu.au/~34829454/assignment-2/user.php" + "?action=edit";
	...
	xhr.send(reqHeader + "&cs_ulog=" + cookie_get("cs_ulog") + "&cs_shoppingcart=" + cookie_get("cs_shoppingcart"));
}
